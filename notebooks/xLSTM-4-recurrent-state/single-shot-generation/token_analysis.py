...
def clean_tokens(tokens: List[str]) -> List[str]:
    ...
    This is applied ONLY as a last resort before MIDI decoding. The grammar
    error count is measured on the ORIGINAL (uncleaned) token stream.
    ...
    while i < n:
        ...
        # Skip malformed or unknown tokens
        if pfx is None or pfx not in known_prefixes:
            i += 1
            continue

        # Complete p-d-v triplet — keep all three
        if pfx == PITCH_PREFIX:
            if (i + 2 < n
                ...
                # Incomplete triplet — skip the pitch token
                i += 1
            continue

        # Skip orphan d-X or v-X (they were part of a broken triplet)
        if pfx in (DUR_PREFIX, VEL_PREFIX):
            i += 1
            continue

        # Everything else (b, o, s, t, i) — keep
        cleaned.append(tok)
        i += 1
