...
# ============================================================================
# FIX #1: REMOVED - Anomaly detection causes 2-5× memory overhead
# ============================================================================
# torch.autograd.set_detect_anomaly(True)  # REMOVED: Causes massive memory usage
# ============================================================================
...
# ============================================================================
# FIX #2: REMOVED - CUDA_LAUNCH_BLOCKING is debug-only, slows training
# ============================================================================
# os.environ["CUDA_LAUNCH_BLOCKING"] = "1"  # REMOVED: Only needed for debugging
# ============================================================================
...
def run_training(config_paths: list[str]):
    ...
    # ============================================================================
    # FIX #3: REPLACED - Custom collator that pads to fixed context_length
    # ============================================================================
    # OLD: data_collator = DataCollatorForLanguageModeling(tokenizer, mlm=False)
    # PROBLEM: Creates variable-length batches, causing unpredictable memory usage
    
    def custom_collate_fn(batch):
        """
        Custom collator that pads all sequences to exactly context_length.
        This ensures predictable memory usage and prevents OOM from variable padding.
        """
        max_len = config.model.context_length
        
        # Extract input_ids and truncate if needed
        input_ids_list = []
        for ex in batch:
            ids = ex['input_ids'][:max_len]  # Truncate to context_length
        ...
        input_ids = torch.nn.utils.rnn.pad_sequence(
            input_ids_list,
            batch_first=True,
            padding_value=fill_token_id
        )
        
        # Ensure all sequences are exactly context_length
    ...
    # ============================================================================
    # FIX #4: ADDED - Gradient checkpointing for 30-50% memory savings
    # ============================================================================
    def enable_gradient_checkpointing(model):
        """
        Enable gradient checkpointing to reduce memory usage by 30-50%.
        Trades memory for computation time (~20% slower training).
        """
        checkpointing_enabled = False
        
        # Try HuggingFace-style gradient checkpointing
        if hasattr(model, 'gradient_checkpointing_enable'):
            try:
                model.gradient_checkpointing_enable()
                accelerator.print("✓ Gradient checkpointing enabled (HuggingFace style)")
                checkpointing_enabled = True
            except Exception as e:
                accelerator.print(f"  Could not enable HF-style checkpointing: {e}")
        
        # Try xLSTM-specific checkpointing
        if hasattr(model, 'xlstm_block_stack'):
            try:
                for i, block in enumerate(model.xlstm_block_stack.blocks):
                    if hasattr(block, 'use_reentrant'):
                        block.use_reentrant = False
                accelerator.print(f"✓ Gradient checkpointing configured for {len(model.xlstm_block_stack.blocks)} xLSTM blocks")
                checkpointing_enabled = True
            except Exception as e:
                accelerator.print(f"  Could not configure xLSTM checkpointing: {e}")
        
        if not checkpointing_enabled:
            accelerator.print("  Note: Gradient checkpointing not available for this model")
        
        return checkpointing_enabled
    
    # Enable gradient checkpointing before compilation
    if config.training.get("gradient_checkpointing", True):
        accelerator.print("Attempting to enable gradient checkpointing...")
    ...
    enable_mixed_precision = config.training.get("enable_mixed_precision", True)
    ...
    for epoch in range(num_epochs):
        ...
        # ============================================================================
        # FIX #5: ADDED - Clear CUDA cache between epochs to prevent fragmentation
        # ============================================================================
        if accelerator.is_local_main_process:
...
def create_readme(output_dir, config):
    ...
    template = template.replace("{{model_name}}", config.training.model_name)
    template = template.replace("{{hugging_face_repo}}", config.training.get("hugging_face_repo", ""))
...
def preprocess_only(config_paths: list[str]):
    """
    Preprocess the dataset and tokenizer only.
    """
    ...
    # Preprocess the dataset and tokenizer.
    accelerator = Accelerator()
    tokenized_datasets, tokenizer = preprocess(config, accelerator, ask_for_overwrite=True)


def preprocess(config, accelerator=None, ask_for_overwrite=False):
    """
    Preprocesses the dataset and tokenizer based on the given configuration.
    """

    # Get the HuggingFace datasets IDs.
    hugging_face_ids = config.dataset.hugging_face_ids

    # Compute the checksum from the config.
    checksum = compute_checksum_from_config(config)

    # Define paths.
    preprocessed_path = os.path.join("preprocessed", checksum)
    data_path = os.path.join(preprocessed_path, "data")
    tokenizer_path = os.path.join(preprocessed_path, "tokenizer")
    tokenized_data_path = os.path.join(preprocessed_path, "tokenized_data")
    checksum_path = os.path.join(preprocessed_path, "checksum.txt")

    # See if the preprocessed data exists.
    if os.path.exists(checksum_path):
        ...
        if existing_checksum == checksum:
            ...
            # If we are asking for overwrite, ask the user.
            if ask_for_overwrite:
                response = input("Preprocessed data already exists. Overwrite? (y/n): ")
                if response.lower() != "y":
                    accelerator.print("Exiting...")
                    sys.exit(0)
                accelerator.print("Overwriting...")

            # If not asking for overwrite, load the preprocessed data.
            else:
                accelerator.print("Loading preprocessed data...")
                
                # Load tokenizer - FIX: Use tokenizer_file directly for custom tokenizers
    ...
    if accelerator.is_local_main_process:
        ...
        if hasattr(config.dataset, 'local_train'):
            accelerator.print("Loading local text files...")
            ...
            raw_datasets = DatasetDict({
                "train": load_text_file(config.dataset.local_train),
                "validation": load_text_file(config.dataset.local_valid),
                "test": load_text_file(config.dataset.local_test)
            })
            accelerator.print(f"Loaded {len(raw_datasets['train'])} train, "
                            f"{len(raw_datasets['validation'])} valid, "
                            f"{len(raw_datasets['test'])} test examples")
        else:
            # Original HuggingFace loading code
    ...
    # Tokenizer creation - FIX: Proper synchronization for distributed training
    if config.tokenizer.type in ["whitespace", "bpe"]:
        if accelerator.is_local_main_process:
            ...
            accelerator.print(f"Tokenizer saved to {tokenizer_path}")
        
        # CRITICAL: Wait for main process to finish saving tokenizer
        accelerator.wait_for_everyone()
        
        # All processes (including main) load the tokenizer the same way
        ...
        # Load tokenizer for all non-main processes
        if not accelerator.is_local_main_process:
